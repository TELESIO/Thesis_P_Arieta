function [displacement] = forward_euler(vertical_Acceleration, Time)
...
for index = 1 : numel(deltaT)-1
    velocity(index+1,1) = velocity(index,1)+vertical_Acceleration(index,1) * deltaT(index,1);
    displacement(index+1,1) = displacement(index,1)+velocity(index,1) * deltaT(index,1);
end

    velocity(numel(velocity),1) = velocity(numel(velocity)-1,1)+vertical_Acceleration(numel(velocity)-1,1) * deltaT(numel(velocity)-1,1);
